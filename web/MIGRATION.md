...
읽는 순서: §1 물건 이해 → §4 불변식 → §6 목표 구조 → §7 작업 단계
...
**남은 것** — §7에 단계별로 있습니다. 저장·동시성 → 게이트웨이 → 화면 → 문제은행 → 배포
...
**프레임워크를 도입할지는 사용자에게 물으세요.** 지금은 없고, 없어도 됩니다.
...
- 단계 전환을 `alarm()`으로. `autoAdvance`의 전이 규칙은 §8-3 그대로 유지
...
- ⚠️ 교사 열쇠 회수 경로가 필요합니다. 앱스 스크립트판은 "시트 메뉴에서 확인"이었는데
  시트를 버리면 그 경로가 없어집니다. **사용자에게 물으세요** (교사 로그인? 이메일 발송?
  판 만들 때 인쇄?)
...
`prefers-reduced-motion`에서도 게임이 그대로 돌 것.
...
- ⚠️ **교사 로그인이 필요한지 사용자에게 물으세요.** 이건 새 요구사항입니다
...
- 일시정지: `pausedAt`을 찍고, 재개할 때 `phaseEndsAt`을 **멈춘 만큼 미룬다**
...
2단계에 들어가기 전에 정해야 합니다.

1. **교사 열쇠 회수 경로** — 시트를 버리면 "메뉴에서 확인"이 사라집니다 (§7-3단계)
2. **문제은행 관리 화면의 인증** — 계정 개념이 새로 필요한가 (§7-5단계)
3. **화면에 프레임워크를 쓸 것인가** — 지금은 없고, 없어도 됩니다 (§6)
