...
- `web/test/gates.ts` 27 · `room.ts` 46 · `gateway.ts` 26 · `admin.ts` 8 · `qr.ts` 10 · `race.ts` 9
  (`parity.ts` 14 는 **리뉴얼로 규칙이 갈라져 폐기(2026-09-06)** — `RENEWAL.md`)
...
> ⚠️ **리뉴얼(2026-09-06)로 바뀐 유일한 불변식입니다 — `RENEWAL.md` §2-2.**
> "참 힌트 27개는 참, 사기 라운드의 3개는 거짓"이 되었고, 문장과 논리식은 이제
> `buildHintPlan` **한 번의 호출**에서 같은 spec 으로 나옵니다 (따로 쓰지 마세요).
...
> ⚠️ **리뉴얼(2026-09-06)로 지키는 방식이 바뀌었습니다 — `RENEWAL.md` §2-2.**
> `hintGiven` 은 없앴습니다. 힌트가 (라운드, 난이도)로 정해지고 한 라운드에 한 번만
> 제출할 수 있으므로(`ALREADY_ANSWERED`) 두 번 받을 길 자체가 없습니다.
> 복구 경로에서 되살릴 상태도 없어졌습니다. 아래 문단은 **옛 구조의 기록**입니다.
...
**리뉴얼로 규칙이 갈라져 폐기(2026-09-06). 지금의 정본은 `RENEWAL.md` 이고,
`src/game/` 은 더 이상 `apps-script/Game.gs` 의 이식본이 아닙니다.**
...
### 판 보존 기간 ✅ (2026-09-07 사용자 결정)

**문제였던 것**: 판이 **영원히 남았습니다.** 한 판은 두 곳에 있는데(D1 `games` 한 줄 +
판마다 하나인 Durable Object 상태), 둘 중 어느 것도 지우는 코드가 없었습니다.
한 학기가 지나면 '최근 판' 표 뒤로 수백 개의 판이 쌓이고, **그 각각이 그 반 학생들의
모둠 암호와 그 판의 정답을 들고 있습니다.**

| | 얼마나 두는가 | 기준 시각 |
|---|---|---|
| 정산이 끝난 판 (`is_over=1`) | **30일** | `finished_at` (정산한 시각) |
| 정산 안 하고 버려진 판 | **90일** | `created_at` |
| 정산했는데 `finished_at` 이 NULL (0007 이전 판) | 30일 | `created_at` 을 대신 쓴다 |

기간을 고치는 곳은 `src/server/cleanup.ts` 의 `RETENTION` **한 곳**입니다.
⚠️ 교사 화면 표 아래의 안내문("정산한 판은 30일…")에 같은 숫자가 적혀 있습니다 —
거기를 고치는 날 그 문장도 같이 고치세요 (§5 `trackCells` 함정과 같은 종류입니다).

**두 가지 경로**

- **자동** — 매일 UTC 18:00(= KST 03:00) `scheduled` 핸들러. `wrangler.jsonc` 의
  `triggers.crons` 가 시각이고, 판단은 `env` 를 모르는 순수 함수(`cleanup.ts`)입니다.
  ⚠️ 한 판이 실패해도 나머지는 계속 지웁니다(판마다 try/catch). 묶어서 감싸면 DO
  하나가 응답하지 않는 날 그날 정리가 통째로 멈추고, 아무도 모른 채 판이 다시 쌓입니다
- **수동** — 교사 화면 '최근 판' 표의 🗑. `POST /api/admin/games/:code/delete`
  (관리자 비밀번호, RENEWAL §3-4). 교사 열쇠로는 못 지웁니다 — 열쇠는 그 판을
  **진행**하는 권한이지 지우는 권한이 아닙니다

**되풀이하기 쉬운 것 세 가지**

1. **D1 만 지우기.** 목록에서는 사라지는데 DO 는 남아서, 코드를 아는 사람이
   `GET /api/game/CODE/lobby` 로 그 판을 계속 만납니다. 반대로 DO 만 지우면
   목록에 죽은 줄이 남아 '이어하기' 가 `GAME_NOT_FOUND` 로 끝납니다
2. **순서를 뒤집기.** 반드시 **DO 먼저, D1 나중**입니다. 뒤집으면 목록에서 줄이
   사라진 뒤 DO 지우기가 실패했을 때 그 판은 어느 목록에도 없으면서 상태만 남아
   **영원히 지워지지 않습니다** (다음 정리가 훑는 것은 `games` 표입니다)
3. **`wipe` 를 `op` 표에 넣기.** `ops.ts` 의 표는 **소켓이 부를 수 있는 이름**입니다.
   거기 넣는 순간 판 코드만 아는 학생이 수업 중인 판을 지웁니다.
   `adminHostKey()` 를 표 밖에 둔 것과 같은 이유입니다 (게이트 `DEL3`)

**게이트** (`test/cleanup.ts` 8개, 가짜 시계): `CLEAN1`(정산 29/30일) ·
`CLEAN2`(미정산 89/90일) · `CLEAN3`(`finished_at` NULL → `created_at`) ·
`CLEAN4`(D1+DO 둘 다 · 하나 실패해도 나머지) · `CLEAN-FIN`(정산이 `finished_at` 을 쓴다) ·
`DEL1`~`DEL3`(관리자만 · 없는 코드 · `op` 표 밖)

로컬 확인:

```bash
npx wrangler dev --test-scheduled
curl "http://localhost:8787/__scheduled?cron=0+18+*+*+*"   # 로그에 [정리] 한 줄
```
...
> ⚠️ **리뉴얼(2026-09-06)로 바뀐 것은 `RENEWAL.md` 를 보세요.**
> `teacherView.className`·`unit` 은 `roomTitle`·`setName` 이 되었고, 두 뷰에 `finished`,
> 모둠 뷰에 `fraudNotice`·`roomTitle`, 교사 뷰에 `fraudEnabled`·`fraudRound`(정산 전 null)가
> 더해졌습니다. `state.finishRound` 는 **어느 뷰에도 나가지 않습니다** (미래가 샙니다).
...
> ⚠️ **리뉴얼(2026-09-06)로 바뀐 것은 `RENEWAL.md` 를 보세요.**
> 라운드는 **10 고정**(`ROUNDS`)이고, 2026-09-07 부터 마지막 라운드(`lastRound`)도 **항상 10** 입니다
> — 골인 라운드를 1위 8R · 2위 9R · 3위 10R 로 고정했기 때문입니다 (RENEWAL §2-1).
> `lastRound` 가 9 인 판은 그 이전에 만들어진 판(이어하기)뿐이고, 그런 판은 지금도 9라운드에서 끝나야 합니다.
> 시간 기본값은 경주 15 · 문제 40 · 토론 90 · 베팅 45초, 시작 코인은 30 입니다.
> 단계 전이 자체는 아래 그림 그대로입니다.
...
`web/` 126개 — `gates.ts` 27 + `room.ts` 46 + `gateway.ts` 26 + `admin.ts` 8 + `qr.ts` 10 + `race.ts` 9.
(`parity.ts` 14 는 리뉴얼로 규칙이 갈라져 폐기 — 2026-09-06)
...
4. ~~`parity.ts`가 깨지면 멈추고 사용자에게 물으세요~~ — **리뉴얼로 폐기(2026-09-06).**
   규칙의 정본은 이제 `RENEWAL.md` 입니다
