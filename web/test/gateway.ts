...
import { RoomOps, THROTTLE } from '../src/do/ops.ts';
...
import type { ApiResponse, DbPort, Ports, PreparedUnit, RecentGame, RoomPort } from '../src/server/ports.ts';

const ORIGIN = 'https://derby.example.workers.dev';
...
const ANIMALS: AnimalRow[] = ANIMAL_CODES.map((c, i) => ({ code: c, name: ANIMAL_NAMES[i]!, emoji: '🐎' }));
...
class MemRoom implements RoomPort {
  room: Room;
  ops: RoomOps;
...
interface CallOpts { body?: unknown; headers?: Record<string, string> }
...
interface Opened { code: string; hostKey: string; pins: Record<number, string>; warnings: string[] }

function dataOf(res: ApiResponse): Record<string, unknown> {
  return (res.body.ok ? res.body.data : {}) as Record<string, unknown>;
}
function errOf(res: ApiResponse): string {
  return res.body.ok ? 'ok' : res.body.error;
}

async function open(net: Net, unit = '유전', teamCount = 6, className = '2학년 3반'): Promise<Opened> {
...
const host = (k: string) => ({ 'X-Host-Key': k });
const pin = (p: string) => ({ 'X-Team-Pin': p });
...
function answerOf(net: Net, code: string, round: number, level: Level): number {
...
let pass = 0, fail = 0;
async function gate(id: string, title: string, fn: () => Promise<{ ok: boolean; detail: string }>) {
  let ok = false, detail = '';
...
console.log('\n=== 게이트웨이 게이트 (인메모리 포트 · 가짜 시계) ===\n');

// ════════════════════════════════════════════════════════════
// 교사 열쇠 — 판 코드는 칠판에 적혀 있어서 비밀이 아니다 (§4-4)
// ════════════════════════════════════════════════════════════
...
console.log('\n====================================================');
console.log(`게이트웨이 게이트 — 통과 ${pass} / 실패 ${fail}`);
console.log('====================================================\n');
