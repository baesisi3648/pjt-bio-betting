...
- [x] **경주 규칙 손보기 (2026-09-07, 동생 피드백)** — 시작 코인 20 → **30**
      (라운드당 3 × 10라운드 = 30. `migrations/0008_coins30.sql` 이 D1 의 값도 같이 올린다),
      골인 라운드 **1위 8R · 2위 9R · 3위 10R 고정**. 예전에는 2·3위가 같은 라운드에
      나란히 들어와 화면에 **공동 2위**로 보였다. 따라오는 것: `lastRound` 는 언제나
      `ROUNDS`(10) 이고(옛 판의 9 는 이어하기로만 남는다), 트랙칸수 상한 26 → **23**
      (1위가 8라운드 안에 못 들어오는 트랙은 판이 성립하지 않는다 — RENEWAL §2-1)
...
```bash
npm test         # 타입 검사 3벌 + 규칙 27 + 방 코어 47 + 게이트웨이 26 + 관리 17 + 정리 8 + QR 10 + 경주 9 = 144
npm run room     # 방 코어만
